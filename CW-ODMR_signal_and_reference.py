...
AOM_CAM_MW = int(0b111000000000000000001101)
...
aom_delay = ctypes.c_double(300*ns)
t_pi = ctypes.c_double(260*ns)

# Rabi cycle parameters


N_avg = 10 # number of times the cycle is going to repeat for each MW_ON_time
start_freq = 2650
end_freq = 3100
Scanning_points = int(end_freq-start_freq+1)

exposure_time = 40 * ms
one_cycle_time = int(aom_time.value + 2*aom_delay.value + t_pi.value)
...
#signal start
signal = pb_inst_pbonly(AOM_CAM_MW,Inst.LOOP,number_of_sequences,aom_delay) # laser delay to turn on
pb_inst_pbonly(AOM_CAM_MW, Inst.CONTINUE, 0, aom_time)
pb_inst_pbonly(AOM_CAM_MW, Inst.CONTINUE, 0, aom_delay)  # laser delay to turn off
pb_inst_pbonly(AOM_CAM_MW, Inst.CONTINUE, 0, t_pi) # time for the rabi pulse
pb_inst_pbonly(AOM_CAM_MW, Inst.END_LOOP, signal, length_ttl) # do i need to have the last 2 lines or just one is enough? I think so cause the length_ttl only happens when continuing to the next inst
