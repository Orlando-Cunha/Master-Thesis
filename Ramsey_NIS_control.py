...
aom_time = ctypes.c_double(800*ns) #100us
...
t_pi2 = ctypes.c_double(160*ns)
# Time parameters

start_Pulse_duration = 200
...
one_cycle_time_with_delay = int(aom_delay.value+aom_time.value+relaxation_time.value+t_pi2.value+end_Pulse_duration+t_pi2.value)
...
for t in Scanned_parameters: # define the number of rabi steps and the values
    ...
    pb_inst_pbonly(MW_CAM, Inst.CONTINUE, 0, t_pi2) # time for pi/2
    ...
    pb_inst_pbonly(CAM, Inst.CONTINUE, 0, aom_delay)  # laser delay to turn off
    #pb_inst_pbonly(CAM, Inst.CONTINUE, 0, relaxation_time)  # relaxation time
    pb_inst_pbonly(CAM, Inst.CONTINUE, 0, t_pi2)  # time for the rabi pulse
    pb_inst_pbonly(CAM, Inst.CONTINUE, 0, t)
    pb_inst_pbonly(CAM, Inst.CONTINUE, 0, t_pi2)  # time for pi/2
    #pb_inst_pbonly(CAM, Inst.CONTINUE, 0, aom_delay)  # laser delay to turn off
    #pb_inst_pbonly(AOM_CAM, Inst.CONTINUE, 0, aom_time)
